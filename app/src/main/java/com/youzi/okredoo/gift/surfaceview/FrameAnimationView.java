...
//        Log.i("ysl","numFrames:" +  numFrames + "|" + lastFrame);
        final long curTime = SystemClock.uptimeMillis();
        if (mStart != 0
                && curTime - mStart >= mDuration) {
            Log.i("ysl","达到了>>" + (curTime - mStart));
...
//        Log.i("ysl","nextFrame:" + nextFrame + "|start:" + start);
        long frameDuration = 0;
        if (nextFrame == mFrameDrawables.size()){
            Log.i("ysl","启动结束！！");
            try {
                Thread.sleep(41);
